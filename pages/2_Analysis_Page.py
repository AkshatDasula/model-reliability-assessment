...
from utils import read_data, new_filter_data, get_drift_dfs, psi, js_num, js_test, psi_num, drift_using_copula
import pandas as pd
import numpy as np
import os
...
with open("./pages/results.json", "r") as af:
    results_dict = json.load(af)
    


if len(st.session_state) == 0:
    print("Session Erased!")
    switch_page("Landing Page")

model_type = st.session_state['model_type']
model_name = st.session_state['model_name']
production_date = st.session_state['production_date']
production_runs_ = st.session_state['all_production_runs']

benchmark_metrics = model_info[model_name]['benchmark_metrics']

st.sidebar.markdown(f"<h3>Current Model:</h3>",unsafe_allow_html=True)
st.sidebar.write(f"Model Name: {model_name}")
st.sidebar.write(f"Production Date: {production_date}")
st.sidebar.write(f"Model Type: {model_type}")

if model_info[model_name]['output_type'] == "Text":
    analysis = ["Performance Drift Analysis", "Input Feature Analysis", "Data Quality Analysis", "Prediction Drift Analysis"]
else:
    analysis = ["Performance Drift Analysis", "Input Feature Analysis", "Data Quality Analysis", "Prediction Drift Analysis", "Model Explanations/Interpretability"]
...
def know_about_metrics(model_type):
    if model_type == "Regression":
        ...
        st.markdown("<p style='font-size: 17px;'>4) <b>ROC AUC</b>: The Receiver Operator Characteristic (ROC) is a probability curve that plots the TPR(True Positive Rate) against the FPR(False Positive Rate) at various threshold values and separates the `signal` from the `noise`. The Area Under the Curve (AUC) is the measure of the ability of a classifier to distinguish between classes. If this metric is 0, there is a possibility that it has only one class, probably after filtering</p>",unsafe_allow_html=True)
...
def Performance_Analysis(true_labels, predicted_labels, model_type):
    ...
    model_type = model_info[model_name]['output_type']
    
    to_analyze = set(true_labels.index).intersection(set(predicted_labels.index))
    true_labels = true_labels.loc[list(to_analyze)]
    predicted_labels = predicted_labels.loc[list(to_analyze)]
    
    extra = true_labels[~true_labels.index.isin(to_analyze)]
    
    # extra = true_labels[~true_labels.index.isin(predicted_labels.index)]
    # true_labels = true_labels[true_labels.index.isin(predicted_labels.index)]
    
    # st.dataframe(predicted_labels, use_container_width=True)
    # st.dataframe(true_labels, use_container_width=True)
    
    # Get intersection of true_labels and predicted_labels indices
    
    
    if len(extra) > 0:
        st.error(f"Indices of records that weren't used in analysis: {len(extra.index)}", icon="⚠️")
    ...
    if model_type =="Text":
        ...
        gt_helper = pd.read_csv(f'./pages/models/{model_name}/helper_target/Ground Truths/{production_date}.csv')
        prod_helper = pd.read_csv(f'./pages/models/{model_name}/helper_target/Production Runs/{production_date}.csv')
        ...
        f1.metric(label="F1", value=round(metrics["F1"],2), delta= str(round(metrics["F1"] - benchmark_metrics["F1"],2)))
        precision.metric(label="Precision", value=round(metrics["Precision"],2), delta= str(round(metrics["Precision"] - benchmark_metrics["Precision"],2)))
        recall.metric(label="Recall", value=round(metrics["Recall"],2), delta= str(round(metrics["Recall"] - benchmark_metrics["Recall"],2)))
        bleu_s.metric(label="BLEU", value=round(metrics["BLEU"],2), delta= str(round(metrics["BLEU"] - benchmark_metrics["BLEU"],2)))
        simi.metric(label="Similarity", value=round(np.mean(prod_helper['similarity_with_gt']),2), delta= str(round(np.mean(prod_helper['similarity_with_gt']) - benchmark_metrics["Similarity"],2)))
        
        st.write("---")
        st.subheader("Benchmark Metrics: ")
        
        f1_bench, precision_bench, recall_bench, bleu_bench, simi_bench = st.columns(5)
        f1_bench.metric(label="F1", value=round(benchmark_metrics["F1"],2))
        precision_bench.metric(label="Precision", value=round(benchmark_metrics["Precision"],2))
        recall_bench.metric(label="Recall", value=round(benchmark_metrics["Recall"],2))
        bleu_bench.metric(label="BLEU", value=round(benchmark_metrics["BLEU"],2))
        simi_bench.metric(label="Similarity", value=round(benchmark_metrics["Similarity"],2))
        ...
        # st.markdown(f"<h4>KS Test with KS statistic: {result_num['ks-stat']} and p-value: {result_num['ks_p-value']}</h4>", unsafe_allow_html=True)
        # if result_num['ks_status'] == True:
        #     st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
        # st.plotly_chart(fig_num)
        
        ks_results, ks_figs = ks_test(prod_helper['similarity_with_gt'], prod_helper['similarity_wth_text'], b_name="Ground Truth")
        psi_num_results, psi_num_figs = psi_num(prod_helper['similarity_with_gt'], prod_helper['similarity_wth_text'], b_name="Ground Truth")
        js_num_results, js_figs = js_num(prod_helper['similarity_with_gt'], prod_helper['similarity_wth_text'])        
                
        
        options = ["KS-Test", "PSI", "JS Divergence"]
        test_type = st.selectbox("Select type of test", options=options, key="test_type")

        if test_type == "KS-Test":
            st.markdown(f"<h4>KS Test with KS statistic: {ks_results['ks-stat']} and p-value: {ks_results['ks_p-value']}</h4>", unsafe_allow_html=True)
            if ks_results['ks_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(ks_figs)
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_num_results['psi_value']} and Threshold: {psi_num_results['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_num_results['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(psi_num_figs, key='psi_n_fig')
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_num_results['jsd_value']} and Threshold: {js_num_results['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_num_results['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            
            st.plotly_chart(js_figs)
    ...
    if model_type == "Classification":
        ...
        actual_true_labels, actual_pred_labels = true_labels.copy(), predicted_labels.copy()
        st.info(f"{list(true_labels.unique())} in Ground Truth and {list(predicted_labels.unique())} in Predicted Labels", icon="ℹ️")
        if type(true_labels.iloc[0]) == str and type(predicted_labels.iloc[0]) == str:
            true_labels, predicted_labels = EncodeLabels(true_labels, predicted_labels)
        
        metrics = classification_metrics(actual_true_labels, actual_pred_labels, get_labels(baseline, model_name), pos_label=sorted(list(actual_true_labels.unique()))[0])
        st.subheader(f"Production Run's Metrics: ")
        f1, precision, recall, roc, accuracy, fpr = st.columns(6)
        f1.metric(label="F1", value=round(metrics["F1"],2), delta= str(round(metrics["F1"] - benchmark_metrics["F1"],2)))
        precision.metric(label="Precision", value=round(metrics["Precision"],2), delta= str(round(metrics["Precision"] - benchmark_metrics["Precision"],2)))
        recall.metric(label="Recall", value=round(metrics["Recall"],2), delta= str(round(metrics["Recall"] - benchmark_metrics["Recall"],2)))
        roc.metric(label="ROC_AUC", value=round(metrics["ROC_AUC"],2), delta= str(round(metrics["ROC_AUC"] - benchmark_metrics["ROC_AUC"],2)))
        accuracy.metric(label="Accuracy", value=round(metrics["Accuracy"],2), delta= str(round(metrics["Accuracy"] - benchmark_metrics["Accuracy"],2)))
        ...
        fpr.metric(label="False Positive Rate", value=round(metrics["False Positive Rate"],2), delta= str(round(benchmark_metrics["False Positive Rate"] - metrics["False Positive Rate"], 2)))
        st.write("---")
        st.subheader("Benchmark Metrics: ")
        ...
        precision_bench.metric(label="Precision", value=round(benchmark_metrics["Precision"],2))
        recall_bench.metric(label="Recall", value=round(benchmark_metrics["Recall"],2))
        roc_bench.metric(label="ROC_AUC", value=round(benchmark_metrics["ROC_AUC"],2))
        accuracy_bench.metric(label="Accuracy", value=round(benchmark_metrics["Accuracy"],2))
        # tpr_bench.metric(label="True Positive Rate", value=round(benchmark_metrics[model]["True Positive Rate"],2))
        fpr_bench.metric(label="False Positive Rate", value=round(benchmark_metrics["False Positive Rate"],2))
        st.write("---")
        
        report = metrics['report']
        st.subheader("Classification Report: ")
        for i in list(actual_true_labels.unique()):
            print(actual_true_labels.unique(), list(report.keys()))
            t_name, precision_t, recall_t, f1_t = st.columns(4)
            with t_name:
                st.markdown(f"<h4>{i}</h4>", unsafe_allow_html=True)
            f1_t.metric(label="F1", value=round(report[str(i)]['f1-score'] * 100, 2))
            precision_t.metric(label="Precision", value=round(report[str(i)]['precision'] * 100, 2))
            recall_t.metric(label="Recall", value=round(report[str(i)]['recall'] * 100, 2))
            st.write("###")
        ...
        with bench_roc:
            ...
            try:
                figure_conf = plot_confusion_matrix(actual_true_labels, actual_pred_labels, get_labels(gt, model_name))
                ...
                st.error("Confusion Matrix not available for this filter", icon="⚠️")
        ...
        with bench_roc:
            ...
            figure_conf = plot_bar(metrics, benchmark_metrics)
            figure_conf.update_layout(
                        #title=f'AUC: {round(roc_bench, 2)}',
                        xaxis_title='Metric',
                        yaxis_title='Value',
    ...
    if model_type == "Regression":
        ...
        r2.metric(label="R2 Score", value=metrics['R2 Score'], delta=round(metrics['R2 Score']-benchmark_metrics['R2 Score'], 2))
        rmse.metric(label="Root Mean Squared Error", value=metrics['Root Mean Squared Error'], delta=round(benchmark_metrics['Root Mean Squared Error'] - metrics['Root Mean Squared Error'], 2))
        mae.metric(label="Mean Absolute Error", value=metrics['Mean Absolute Error'], delta=round(benchmark_metrics['Mean Absolute Error'] - metrics['Mean Absolute Error'], 2))
        mape.metric(label="Mean Absolute Percentage Error", value=metrics['Mean Absolute Percentage Error'], delta=round(benchmark_metrics['Mean Absolute Percentage Error'] - metrics['Mean Absolute Percentage Error'], 2))

        st.write("---")
        st.subheader(f"Benchmark Metrics: ")
        b_r2, b_rmse, b_mae, b_mape = st.columns(4)
        b_r2.metric(label="R2 Score", value=benchmark_metrics['R2 Score'])
        b_rmse.metric(label="Root Mean Squared Error", value=benchmark_metrics['Root Mean Squared Error'])
        b_mae.metric(label="Mean Absolute Error", value=benchmark_metrics['Mean Absolute Error'])
        b_mape.metric(label="Mean Absolute Percentage Error", value=benchmark_metrics['Mean Absolute Percentage Error'])
...
def Data_Drift_Analysis(baseline, production, model_type, production_run, model_name):
    ...
    if model_type == "Computer Vision (CV)":
        ...
        baseline['Anomaly']  = baseline['Anomaly'].astype(str)
        production['Anomaly']  = production['Anomaly'].astype(str)        
    ...
    if model_type == "Natural Language Processing (NLP)":
        ...
        ks_results, ks_figs = ks_test(list(sorted_words_baseline.values())[:1000], list(sorted_words_production.values())[:1000], nlp=True)
        psi_num_results, psi_num_figs = psi_num(pd.Series(list(sorted_words_baseline.values())[:1000]), pd.Series(list(sorted_words_production.values())[:1000]), nlp=True)
        js_num_results, js_figs = js_num(pd.Series(list(sorted_words_baseline.values())[:1000]), pd.Series(list(sorted_words_production.values())[:1000]), nlp=True)
        
        
        # st.markdown(f"<h4>KS Test with KS statistic: {freq_result['ks-stat']} and p-value: {freq_result['ks_p-value']}</h4>", unsafe_allow_html=True)
        # if freq_result['ks_status'] == True:
        #     st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
    ...
    drift_num_val, drift_cat_val = 0, 0
    drift_df_num, drift_df_cat = pd.DataFrame(), pd.DataFrame()
    
    if len(numerical_ft) != 0:
        
        ks_results , ks_figs = ks_test(baseline, production, num_ft = numerical_ft)
        psi_num_results, psi_num_figs = psi_num(baseline, production, num_ft = numerical_ft)
        js_num_results, js_num_figs = js_num(baseline, production, num_ft = numerical_ft)
        
        # st.write(ks_results)
        
        drift_df_num, drifting_nums = get_drift_dfs("numerical", ks_results=ks_results,  
                                        psi_num_results=psi_num_results, 
                                         js_num_results=js_num_results, 
                                        num_ft=numerical_ft)
        
        drift_num_val = len(drifting_nums)
        
    if len(categorical_ft) != 0:
        
        chi_results, chi_figs = chi_test(baseline, production, categorical_ft=categorical_ft)
        psi_results, psi_figs = psi(baseline, production, categorical_ft=categorical_ft)
        js_results, js_figs = js_test(baseline, production, features=categorical_ft)
        
        drift_df_cat, drifting_cats = get_drift_dfs("categorical", chi_results=chi_results,  
                                        psi_results=psi_results, 
                                         js_results=js_results, 
                                        cat_ft=categorical_ft)
        
        drift_cat_val = len(drifting_cats)
    ...
    if model_type != "CV":
        st.info("🛈 Note: To check feature importance of the drifting features please go to SHAP Analysis section.")
    ...
    feature_selection, test_selection = st.columns(2)
    with feature_selection:
        feature_name = st.selectbox("Select feature to analyse drift", options=baseline.columns)
    
    with test_selection:
        options = ["KS-Test", "PSI", "JS Divergence"] if feature_name in numerical_ft else ["Chi-Square Test", "PSI", "JS Divergence"]
        test_type = st.selectbox("Select type of test", options=options)

    if feature_name in numerical_ft:
        
        if test_type == "KS-Test":
            st.markdown(f"<h4>KS Test with KS statistic: {ks_results[feature_name]['ks-stat']} and p-value: {ks_results[feature_name]['ks_p-value']}</h4>", unsafe_allow_html=True)
            if ks_results[feature_name]['ks_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(ks_figs[feature_name])
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_num_results[feature_name]['psi_value']} and Threshold: {psi_num_results[feature_name]['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_num_results[feature_name]['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(psi_num_figs[feature_name], key='psi_n_fig')
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_num_results[feature_name]['jsd_value']} and Threshold: {js_num_results[feature_name]['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_num_results[feature_name]['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(js_num_figs[feature_name])
    
    elif feature_name in categorical_ft:
        
        if test_type == "Chi-Square Test":
            
            st.markdown(f"<h4>Chi Square Test with Chi statistic: {chi_results[feature_name]['chi']} and p-value: {chi_results[feature_name]['chi_p-value']}</h4>", unsafe_allow_html=True)
            if chi_results[feature_name]['chi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(chi_figs[feature_name])
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_results[feature_name]['psi_value']} and Threshold: {psi_results[feature_name]['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_results[feature_name]['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(psi_figs[feature_name])
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_results[feature_name]['jsd_value']} and Threshold: {js_results[feature_name]['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_results[feature_name]['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(js_figs[feature_name])
    
    st.write("---")
    st.subheader("Drift in Feature Relationships using Copula")
        
    try:
        max_ft, drift_ft, heat_figure = drift_using_copula(baseline, production)
        st.write("Top 3 pairs of features whose relationship has changed are:")
        for i in max_ft:
            st.write(f"{i}")
        
        st.write("Pairs of features whose relationship has drifted under confidence threshold 0.001:")
        if len(drift_ft) == 0:
            st.success("No feature pairs have drifted under confidence threshold 0.001", icon="👍")
        else:
            for i in drift_ft:
                st.write(f" {i}")
        
        # st.markdown(f"<h4>Heatmap of Copula Drift</h4>", unsafe_allow_html=True)
        heat_figure.update_layout(width=1000,
                                height=500,
                                title="Heatmap of Copula Drift",
                                title_font={"size": 20},
                                # yaxis_title='Count',
                                xaxis_title_font={"size":16, "color":"black"},
                                yaxis_title_font={"size":16, "color":"black"})
        ...
        st.warning("Copula Drift Analysis not available", icon="⚠️")
...
def Data_Quality_Analysis(production_data):
    ...
    if model_type == "Computer Vision (CV)":
        df = pd.read_csv(f"./pages/models/{model_name}/Production/{production_date}/production_paths_df.csv")
        ...
        st.markdown(f"<h4>Number of Anomalies in {production_date}: <u>{np.sum(df['Anomaly'] == True)}</u></h4>", unsafe_allow_html=True)
        ...
        if model_type == "Natural Language Processing (NLP)":
            quality_type = st.sidebar.selectbox("Select a Data Quality metric",options=['Completeness','Uniqueness','NLP Metrics'])
        else:
            quality_type = st.sidebar.selectbox("Select a Data Quality metric",options=['Completeness','Uniqueness','Validity'])
        ...
        if quality_type == "Completeness":
            ...
            total, complete, missing = st.columns(3)
            ...
            fig_donut.update_layout(title=f'Uniqueness Score: {score}%',
                                        ...
                                        height=400)
            st.plotly_chart(fig_donut)
            total, unique, duplicate = st.columns(3)
        ...
        #         st.markdown("<p style='font-size: 17px;'><b>Profile Report: </b>Profile reports provide a comprehensive summary of dataset characteristics, including basic statistics, distributions, and data quality metrics. Profile reports help data analysts quickly grasp the nature of the dataset, its key attributes, and potential data quality issues. Visualizations such as histograms, scatter plots, and correlation matrices in profile reports facilitate better understanding and analysis. Profile reports highlight missing values, duplicates, unique values, and other key metrics critical for data quality assessment. Profile reports serve as a starting point for data exploration, guiding data preprocessing, validation, and feature engineering efforts.",unsafe_allow_html=True)
            ...
            total_val, invalid, valid = st.columns(3)  
            ...
            production_helper = pd.read_csv(f"./pages/models/{model_name}/helper/{production_date}.csv")
            ...
            st.write("---")
            st.markdown(f"<h2><u>Spelling Errors in {production_date}</u></h2>", unsafe_allow_html=True)
...
def Prediction_Drift_Analysis(baseline, production, model_type, production_run):
    ...
    if model_type == "Text":
        ...
        # st.markdown(f"<h4>KS Test with KS statistic: {result_read['ks-stat']} and p-value: {result_read['ks_p-value']}</h4>", unsafe_allow_html=True)
        # if result_read['ks_status'] == True:
        #     st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
        # st.plotly_chart(fig_read)
        
        ks_read_results, ks_read_figs = ks_test(gt_helper['readability'], prod_helper['readability'], b_name="Ground Truth")
        psi_read_results, psi_read_figs = psi_num(gt_helper['readability'], prod_helper['readability'], b_name="Ground Truth")
        js_read_results, js_read_figs = js_num(gt_helper['readability'], prod_helper['readability'])    
        
        options = ["KS-Test", "PSI", "JS Divergence"]
        test_type = st.selectbox("Select type of test", options=options, key="test_type")

        if test_type == "KS-Test":
            st.markdown(f"<h4>KS Test with KS statistic: {ks_read_results['ks-stat']} and p-value: {ks_read_results['ks_p-value']}</h4>", unsafe_allow_html=True)
            if ks_read_results['ks_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(ks_read_figs)
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_read_results['psi_value']} and Threshold: {psi_read_results['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_read_results['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(psi_read_figs, key='psi_n_fig')
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_read_results['jsd_value']} and Threshold: {js_read_results['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_read_results['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            
            st.plotly_chart(js_read_figs)
        ...
        # result_length, fig_length = ks_test()
        # st.markdown(f"<h4>KS Test with KS statistic: {result_length['ks-stat']} and p-value: {result_length['ks_p-value']}</h4>", unsafe_allow_html=True)
        # if result_length['ks_status'] == True:
        #     st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
        # st.plotly_chart(fig_length)
        
        
        ks_len_results, ks_len_figs = ks_test(gt_helper['length'], prod_helper['length'], b_name="Ground Truth")
        psi_len_results, psi_len_figs = psi_num(gt_helper['length'], prod_helper['length'], b_name="Ground Truth")
        js_len_results, js_len_figs = js_num(gt_helper['length'], prod_helper['length'])    
        
        options = ["KS-Test", "PSI", "JS Divergence"]
        test_type = st.selectbox("Select type of test", options=options, key="lenght_test_type")

        if test_type == "KS-Test":
            st.markdown(f"<h4>KS Test with KS statistic: {ks_len_results['ks-stat']} and p-value: {ks_len_results['ks_p-value']}</h4>", unsafe_allow_html=True)
            if ks_len_results['ks_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(ks_len_figs)
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_len_results['psi_value']} and Threshold: {psi_len_results['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_len_results['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(psi_len_figs, key='psi_n_fig')
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_len_results['jsd_value']} and Threshold: {js_len_results['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_len_results['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            
            st.plotly_chart(js_len_figs)
        ...
        # result_pred, fig_pred = ks_test()
        # st.markdown(f"<h4>KS Test with KS statistic: {result_pred['ks-stat']} and p-value: {result_pred['ks_p-value']}</h4>", unsafe_allow_html=True)
        # if result_pred['ks_status'] == True:
        #     st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
        # st.plotly_chart(fig_pred)
        
        
        ks_sim_results, ks_sim_figs = ks_test(gt_helper['similarity'], prod_helper['similarity_wth_text'], b_name="Ground Truth")
        psi_sim_results, psi_sim_figs = psi_num(gt_helper['similarity'], prod_helper['similarity_wth_text'], b_name="Ground Truth")
        js_sim_results, js_sim_figs = js_num(gt_helper['similarity'], prod_helper['similarity_wth_text'])    
        
        options = ["KS-Test", "PSI", "JS Divergence"]
        test_type = st.selectbox("Select type of test", options=options, key="sim_test_type")

        if test_type == "KS-Test":
            st.markdown(f"<h4>KS Test with KS statistic: {ks_sim_results['ks-stat']} and p-value: {ks_sim_results['ks_p-value']}</h4>", unsafe_allow_html=True)
            if ks_sim_results['ks_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(ks_sim_figs)
        
        if test_type == "PSI":
            st.markdown(f"<h4>PSI with PSI value: {psi_sim_results['psi_value']} and Threshold: {psi_sim_results['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_sim_results['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
           
            st.plotly_chart(psi_sim_figs, key='psi_n_fig')
        
        if test_type == "JS Divergence":
            st.markdown(f"<h4>JSD with value: {js_sim_results['jsd_value']} and Threshold: {js_sim_results['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_sim_results['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            
            st.plotly_chart(js_sim_figs)
        ...
        if model_type == "Regression":
            ...
            ks_results , ks_figs = ks_test(baseline_, production_)
            psi_num_results, psi_num_figs = psi_num(baseline_, production_)
            js_num_results, js_num_figs = js_num(baseline_, production_)
            ...
            st.plotly_chart(js_num_figs, key='js_n_fig')
        ...
        if model_type == "Classification":
            ...
            chi_results, chi_figs = chi_test(baseline_, production_)
            psi_results, psi_figs = psi(baseline_, production_)
            js_results, js_figs = js_test(baseline_, production_)
            
            st.markdown(f"<h4>Chi Square Test with Chi statistic: {chi_results['chi']} and p-value: {chi_results['chi_p-value']}</h4>", unsafe_allow_html=True)
            if chi_results['chi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(chi_figs)
        
            st.write("---")
            
            st.markdown(f"<h4>PSI with PSI value: {psi_results['psi_value']} and Threshold: {psi_results['psi_threshold']}</h4>", unsafe_allow_html=True)
            if psi_results['psi_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(psi_figs, key='psi_fig')
            
            st.write("---")
            
            st.markdown(f"<h4>JSD with value: {js_results['jsd_value']} and Threshold: {js_results['js_threshold']}</h4>", unsafe_allow_html=True)
            if js_results['js_status'] == True:
                st.error(f"⚠️ Drift experienced, Consider looking into data or retrain the model with new data")
            st.plotly_chart(js_figs, key='js_fig')
        ...
        # for i in prev_runs:
        #     _, prods,_ = read_data(model_name, i)
...
def SHAP_Analysis(baseline, production, model_type, model_name, scaler=None):
    ...
    if model_type == "Computer Vision (CV)":
        ...
        class_names = os.listdir(f"{path}/{production_date}")
        random_images = dict()
        for i in class_names[:-1]:
            random_images[i] = f"{path}/{production_date}/{i}/" + random.sample(os.listdir(f"{path}/{production_date}/{i}"), 1)[0]
        ...
        df = pd.read_csv(f"{path}/Production Runs/{production_date}.csv")
        ...
        shap.initjs()
        cat_has_int, miss_dict, score, indices, outliers_index, num_invalid = validity_check(baseline, production_data_)
        if (len(indices) == 0) or (score == 100):
            pass
        else:
        ...
        if model_type == "Regression":
            ...
            with b_waterfall:
                ...
                shap.plots.waterfall(shap_values_baseline[row])
                st.pyplot(figure1)
            with p_waterfall:
                figure2, ax = plt.subplots()
                shap.plots.waterfall(shap_values_production[row])
                st.pyplot(figure2)
            ...
            with b_bar:
                ...
                shap.plots.bar(shap_values_baseline)
                st.pyplot(figure3)
            with p_bar:
                figure4, ax = plt.subplots()
                shap.plots.bar(shap_values_production)
                st.pyplot(figure4)
            ...
            with b_violin:
                ...
                shap.plots.violin(shap_values_baseline[:100])
                st.pyplot(figure5)
            with p_violin:
                figure6, ax = plt.subplots()
                shap.plots.violin(shap_values_production[:100])
                st.pyplot(figure6)
            ...
            with b_decision:
                figure7, ax = plt.subplots()
                shap.plots.decision(explainer.expected_value, explainer.shap_values(baseline_one)[:5], feature_names=list(baseline_one.columns))
                st.pyplot(figure7)
            with p_decision:
                figure8, ax = plt.subplots()
                shap.plots.decision(explainer.expected_value, explainer.shap_values(production_one)[:5], feature_names=list(baseline_one.columns))
                st.pyplot(figure8)
...
gt, production_data_, baseline = read_data(model_name, production_date)
cat_ft, num_ft = determine_dtype_ft(baseline)

if model_type != "Computer Vision (CV)":

    cols = st.sidebar.multiselect("Select columns to filter data:", options=list(baseline.columns))

    num_conds, cat_conds, text_conds = dict(), dict(), dict()
    ...
    if model_type == "Natural Language Processing (NLP)":
        ...
        if model_info[model_name]['output_type'] == "Text":
            cat_ft.remove('target')

    if len(cols) > 0:
        for i in cols:
            # cat_ft.remove('text') if model_type == "Natural Language Processing (NLP)" else cat_ft
            if i in num_ft: 
                limit = st.sidebar.text_input(f"Filter {i} (Enter values as e.g Age: 18-36)", key=f"key_{i}")
                num_conds[i] = limit
            if i in cat_ft:
                col_option = st.sidebar.multiselect(f"Choose Option for {i}:", options=list(baseline[i].unique()), key=f"key_{i}")
                cat_conds[i] = col_option
            
            if model_type == "Natural Language Processing (NLP)":
                condition = [model_info[model_name]['text_column'], 'target'] if model_info[model_name]['output_type'] == "Text" else [model_info[model_name]['text_column']]
                if i in condition:
                    words = st.sidebar.text_input(f"Filter {i} (Enter words separated by commas to filter. Eg: hello,world)", key=f"key_3")
                    text_conds[i] = words.split(",")
                
        
        if st.sidebar.button("Submit"):
            
            baseline = new_filter_data(baseline, num_conds, cat_conds, text_conds)
            production_data_ = new_filter_data(production_data_, num_conds, cat_conds, text_conds)
            gt = new_filter_data(gt, num_conds, cat_conds, text_conds)
            st.sidebar.success(f"Filter applied!, {len(baseline)} records fetched in baseline and {len(production_data_)} records fetched in production data")
            
                
    else:
        st.sidebar.warning("No Filter applied!, Using full dataset for analysis")
            

if analysis_type == "Performance Drift Analysis":
    Performance_Analysis(gt['target'], production_data_['target'], model_type)
elif analysis_type == "Input Feature Analysis":
    Data_Drift_Analysis(baseline.drop('target',axis=1), production_data_.drop('target',axis=1), model_type, production_date, model_name)
elif analysis_type == "Data Quality Analysis":
    Data_Quality_Analysis(production_data_[gt.drop('target',axis=1).columns])
elif analysis_type == "Prediction Drift Analysis":
    Prediction_Drift_Analysis(gt, production_data_, model_info[model_name]['output_type'], production_date)
elif analysis_type == "Model Explanations/Interpretability":
    SHAP_Analysis(baseline.drop('target',axis=1), production_data_.drop('target',axis=1), model_type, model_name)
